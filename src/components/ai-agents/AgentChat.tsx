  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('agent_messages').select('*').eq('conversation_id', conversationId).order('created_at', {
        ascending: true
      });
      if (error) {
  ...
  const loadConversationTitle = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('agent_conversations').select('title').eq('id', conversationId).maybeSingle();
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4 flex-shrink-0 bg-neutral-950">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            {isEditingTitle ? <div className="flex items-center gap-2 flex-1">
                <Input value={editTitleValue} onChange={e => setEditTitleValue(e.target.value)} onKeyDown={handleKeyPress} className="bg-neutral-900 border-neutral-700 text-white flex-1" autoFocus />
                <Button onClick={handleSaveTitle} size="sm" variant="ghost" className="text-green-400 hover:text-green-300 h-8 w-8 p-0">
                  <Check className="w-4 h-4" />
                </Button>
                <Button onClick={handleCancelEdit} size="sm" variant="ghost" className="text-red-400 hover:text-red-300 h-8 w-8 p-0">
                  <X className="w-4 h-4" />
                </Button>
              </div> : <>
                ...
                {conversationId && <Button onClick={handleEditTitle} size="sm" variant="ghost" className="text-neutral-400 hover:text-white h-8 w-8 p-0" title="Editar título">
                    <Pen className="w-4 h-4" />
                  </Button>}
              </>}
          </div>
          <Button onClick={createNewConversation} variant="outline" size="sm" className="border-neutral-700 text-neutral-300 hover:bg-neutral-800 flex-shrink-0 ml-4">
        ...
        <CardContent className="flex flex-col flex-1 p-0 overflow-hidden bg-neutral-950">
          <ScrollArea className="flex-1 px-6" ref={scrollAreaRef}>
            <div className="space-y-4 py-4 min-h-full">
              {messages.length === 0 ? <div className="text-center text-neutral-400 py-8">
          ...
          <div className="p-6 border-t border-neutral-800 flex-shrink-0 bg-neutral-950">
            <div className="flex gap-2 items-end">
              <Textarea value={input} onChange={e => setInput(e.target.value)} onKeyPress={handleInputKeyPress} placeholder="Digite sua mensagem..." className="flex-1 min-h-[60px] max-h-[120px] bg-neutral-900 border-neutral-700 text-white placeholder-neutral-400 resize-none" disabled={loading} />
              <Button onClick={sendMessage} disabled={!input.trim() || loading} className="h-[60px] px-4 flex-shrink-0 bg-slate-50">
