...
export const AdsTab: React.FC = () => {
  ...
  const collectAllData = async () => {
    try {
      ...
      console.log('📊 Collecting all data for ads tab...');

      // Get all active training data for this tab
      ...
      console.log('📋 Found training data items:', allData?.length || 0);
      ...
      console.log('📝 Manual prompt:', promptData ? 'Present' : 'None');

      return {
        manual_prompt: promptData?.manual_prompt || '',
        files: files,
        links: links
      };
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-medium text-white">Anúncios</h3>
            <p className="text-sm text-neutral-400">
              Configure materiais e instruções para criação de anúncios
            </p>
          </div>
        </div>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Adicione informações específicas sobre anúncios, instruções especiais, ou conhecimento que o agente deve ter..."
              className="bg-neutral-800 border-neutral-700 text-white resize-none"
              rows={6}
            />
            <p className="text-xs text-neutral-500">
